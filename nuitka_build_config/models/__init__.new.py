...
def extra(option_name: str, **kwargs):
    kwargs.update(option_name=option_name)
    return kwargs


class Includes(BaseModel):
    packages: StrList = Field(default_factory=list, json_schema_extra=extra('--include-package'),
                              description=gettext("List of packages to include in the build (--include-package)"))
    modules: StrList = Field(default_factory=list, json_schema_extra=extra('--include-module'),
                             description=gettext("List of modules to include in the build (--include-module)"))
    package_data: StrList = Field(default_factory=list, json_schema_extra=extra('--include-package-data'),
                                  description=gettext("Packages whose data files should be included (--include-package-data)"))
    files: FilesList = Field(default_factory=list, json_schema_extra=extra('--include-data-files'),
                             description=gettext("File patterns to include (--include-data-files)"))
    directories: FilesList = Field(default_factory=list, json_schema_extra=extra('--include-data-dir'),
                                 description=gettext("Data directories to include (--include-data-dir)"))
    noinclude_data_files: StrList = Field(default_factory=list, json_schema_extra=extra('--noinclude-data-files'),
                                          description=gettext("File patterns to exclude from data (--noinclude-data-files)"))


class VersionInfo(BaseModel):
    company_name: NullStr = Field(default=None, json_schema_extra=extra('--company-name'),
                                  description=gettext("Company name in version info (--company-name)"))
    product_name: NullStr = Field(default=None, json_schema_extra=extra('--product-name'),
                                  description=gettext("Product name in version info (--product-name)"))
